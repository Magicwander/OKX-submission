...
export default class KeeperService {
    ...
    try {
      ...
      for (const [token, priceData] of prices) {
        if (priceData.length > 0) {
          ...
          const avgPrice = priceData.reduce((sum, data) => sum + data.price, 0) / priceData.length;
   ...
   */
  shouldUpdatePrice(token, newPrice) {
    ...
    const priceChange = Math.abs(newPrice.minus(lastPrice.price).dividedBy(lastPrice.price).toNumber());
  ...
  async createPriceUpdateTransaction(token, price) {
    ...
    if (this.config.oracleProgram && this.config.priceAccounts.has(token)) {
      // Create oracle update instruction
      const priceAccount = this.config.priceAccounts.get(token);
      
      // Convert price to appropriate format (e.g., scaled integer)
      const scaledPrice = price.mul(1000000).floor(); // 6 decimal places
      ...
      const updateInstruction = new TransactionInstruction({
        ...
        programId: new PublicKey(this.config.oracleProgram),
        data: Buffer.concat([
          Buffer.from([0]), // Update price instruction
          Buffer.from(token, 'utf8'),
          Buffer.alloc(8)
        ])
      });
      
      // Write price as little-endian 64-bit integer
      updateInstruction.data.writeBigUInt64LE(BigInt(scaledPrice.toString()), updateInstruction.data.length - 8);
